...
{
	//print_info();
	DebugPrintf("render_init : print_info() \n");
	detect_caps();
	DebugPrintf("render_init : detect_caps() \n");
	if(!set_defaults()) return false;
	DebugPrintf("render_init : set_defaults() \n");
	resize_viewport(info->ThisMode.w, info->ThisMode.h);
	DebugPrintf("render_init : resize_viewport() \n");
	if(info->wireframe)

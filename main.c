...
// breakpad running through wine, built for windows doens't work well..
	...
	svcOutputDebugString("lua_init()",20);
	
	// copy game settings from config
	GetGamePrefs();
	
	svcOutputDebugString("GetGamePrefs()",20);
	...
	svcOutputDebugString("ParseCommandLine()",20);
	//
	...
	svcOutputDebugString("sdl_init_video()",20);
...
// this is where it starts to take so long cause it scans directory for dynamic sound files...
	...
	svcOutputDebugString("InitScene()",20);
	...
	svcOutputDebugString("InitView()",20);

...
bool sdl_init( void )
{
	...
	{
		DebugPrintf("Failed to initialize sdl: %s\n",SDL_GetError());
		return false;
	}
...
{
	DebugPrintf("set_opengl_settings\n");
#ifdef GL
	// BPP should be left alone to match whatever the desktop is at.
...
#else
	u_int32_t window_flags = SDL_WINDOW_FULLSCREEN;
#endif	
...
{
	DebugPrintf("entered  sdl_init_video()\n");
	
#if SDL_VERSION_ATLEAST(2,0,0)
	u_int32_t renderer_flags = create_renderer_flags();
	DebugPrintf("sdl_init_video : create_renderer_flags()\n");
	u_int32_t window_flags   = create_window_flags();
	DebugPrintf("sdl_init_video : create_window_flags()\n");
#else
	...
	DebugPrintf("sdl_init_video : set_window_icon()\n");

	set_opengl_settings();
	
	DebugPrintf("sdl_init_video : set_opengl_settings()\n");

	init_video_modes( window_flags );
	
	DebugPrintf("sdl_init_video : init_video_modes()\n");

	set_aspect_ratio();
	
	DebugPrintf("sdl_init_video : set_aspect_ratio()\n");

	if(!create_video_surface( window_flags, renderer_flags ))
		return false;
	
	
	DebugPrintf("sdl_init_video : create_video_surface()\n");
	...
	DebugPrintf("sdl_init_video : set_window_title()\n");
	...
	DebugPrintf("sdl_init_video : render_init()\n");
